...
int test_casadi_mpc() {
    ...
    const double inputTranslationVariationLimit = 5e-4;
    const double inputRotationVariationLimit    = 5e-3;
    const double boundaryConditionsMaxNorm = 1e-5;
    const double initUnknowVarBoxDim = 1;
    ...
    for (int k = 0; k < N; ++k) {
        ...
        // obj += casadi::MX::mtimes(du.T(), casadi::MX::mtimes(1000 * R, du));
    ...
    for (int k = 0; k < N; ++k) {
        std::vector<casadi::MX> args = { U(casadi::Slice(), k), initUnknownVar(casadi::Slice(), k) };
        ...
        casadi::MX bc     = funValue(casadi::Slice(nx+4, nx + bcNu));         // bcNu x 1
