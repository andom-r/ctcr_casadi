...
struct MpcConfig {
    ...
    double inputRotationVariationLimit    =2e-3;
...
static MpcProblem buildMpcProblem(CtrModel& ctr, const MpcConfig& cfg) {
    ...
    for (int k = 0; k < N; ++k) {
        ...
        // objective
        obj += xQX;
        obj += casadi::MX::mtimes(uDiff.T(), casadi::MX::mtimes(0.001 * R, uDiff));;
...
int test_casadi_mpc() {
    ...
    Eigen::Vector3d Pdes = P + Eigen::Vector3d(-40e-3, 0, 0);
    ...
    for (int j = 1; j < (int)points.size() - 1; ++j) {
        ...
        while (((pDesEigen - P).norm() > epsilon) && convergenceIndex < 50) {
