...
namespace CtrLib{
  struct parameters{
      ...
      Vector_X Tref;         // Homogeneous transformation matrix between the reference sensor frame and the robot bas frame [m for the translation part]
      ...
      Vector_q offset;       // Translational and rotational offsets (between the "actuator zero position" and the "zero position" defined in the model) [m ; m; m; deg; deg; deg]
  ...
  void printParameter(const parameters &p);
  int loadParameters (const std::string &path, std::vector<parameters> &vParameters_out);
}
